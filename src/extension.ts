...
export function activate(context: vscode.ExtensionContext) {
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "frontmatterStudio.openEditor",
      () => openEditor(context)
    ),
    vscode.commands.registerCommand(
      "frontmatterStudio.validate",
      () => validate()
    )
  );
}

function currentDocument(): vscode.TextDocument | undefined {
 ...
 */
function workspaceRootForDocument(
  doc: vscode.TextDocument
    ...
    async (message) => {
      ...
      if (message.type === "save") {
        try {
          ...
          if (!applied) {
            throw new Error(
              "VS Code could not apply the changes."
            );
          }
      ...
      if (message.type === "pickThumbnail") {
        ...
        if (
          ...
          vscode.window.showWarningMessage(
            "Please select an image inside the current workspace."
          );
...
async function validateDocument(
  ...
  try {
    ...
    vscode.window.showInformationMessage(
      `Frontmatter is valid: ${path.basename(
        documentUri.fsPath
      )}`
    );
  } catch (error) {
